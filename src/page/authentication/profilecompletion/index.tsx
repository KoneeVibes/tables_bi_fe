...
import { completeUserProfile } from "../../../util/authentication/completeProfile";
...
export const ProfileCompletion = () => {
    ...
    const handleNavigateToPrevious = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        e.preventDefault();
        setError(null);
    ...
    const handleCompleteUserProfile = async () => {
        try {
            const response = await completeUserProfile(formDetails, id);
            if (response.status === "success") {
                setIsLoading(false);
                navigate("/");
            } else {
                setIsLoading(false);
                setError('Profile completion failed. Please check your credentials and try again.');
            }
        } catch (error: any) {
            setIsLoading(false);
    ...
    return (
        <AuthLayout>
            ...
            >
                ...
                >
                    ...
                    >
                        <Box
                            ...
                            sx={{
                                width: `${((profileCompletionActiveTabIndex + 1) / steps.length) * 100}%`,
                                transition: "width 0.5s cubic-bezier(0.4, 0, 0.2, 1)",
                            }}
                        />
                    </Stack>
                    <Stack
                        className="stepper-progress-detail"
                    >
                        <Box
                            overflow={"hidden"}
                        >
                ...
                <Stack
                    className="call-to-action-stack"
                >
                    <Box
                        overflow={"hidden"}
                    >
                        ...
                        >
                            {isLoading ? (
                                ...
                                >
                                    {profileCompletionActiveTabIndex === 2 ? "Finish"
                                        : "Continue"}
                                </Typography>
                            )}
                        </BaseButton>
                    ...
                    >
                        ...
                        >
                            <Typography
                                ...
                                textTransform={"inherit"}
                            >
                                {profileCompletionActiveTabIndex === 0 ? "Skip"
                                    : "Previous"}
                            </Typography>
                        </BaseButton>
                    </Box>
