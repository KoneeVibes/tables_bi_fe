...
export const SignUp = () => {
	const { signUpActiveTabIndex, setSignUpActiveTabIndex } =
	...
	const handleSignUpUser = async (
		...
		try {
			...
			if (response.status === "success") {
				tabIndex === 0.1 ? setIsResendOTPLoading(false) : setIsLoading(false);
	...
	const handleVerifyOtp = async (otp: Record<string, any>) => {
		try {
			...
			if (response.status === "success") {
				...
				setIsLoading(false);
				setError(
					"OTP verification failed. Please check your credentials and try again."
				);
			}
		} catch (error: any) {
			setIsLoading(false);
	...
	const handleFormSubmit = async (
		...
		e.preventDefault();
		setError(null);
		tabIndex === 0.1 ? setIsResendOTPLoading(true) : setIsLoading(true);
		let payload;
		switch (tabIndex) {
			case 0:
			case 0.1:
			...
			>
						...
						{hasOTPResent ? (
							...
							>
								...
								<Typography
									...
									sx={{ cursor: "pointer" }}
									onClick={handleResendOTP}
									visibility={isResendOTPLoading ? "hidden" : "visible"}
								>
									Resend OTP
								</Typography>
							</Typography>
				...
				<Box overflow={"hidden"}>
					...
					>
						...
						>
							{signUpActiveTabIndex === 0 ? "Create Account" : "Verify"}
						</Typography>
					</BaseButton>
				</Box>
			</SignUpWrapper>
